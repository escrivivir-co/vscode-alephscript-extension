...
export class UIManager {
    private uis: Map<string, UIInstance> = new Map();

    constructor(private processManager: ProcessManager) {}

    public async showUIManager() {
        ...
        try {
            ...
            if (configPath) {
                ...
                if (configData.ui && Array.isArray(configData.ui)) {
                    configData.ui.forEach((uiConfig: any) => {
                        const ui: UIInstance = {
                            id: uiConfig.id,
                            name: uiConfig.name,
                            type: uiConfig.type,
    ...
    private async startUI(uiId: string) {
        try {
            ...
            uiProcess.on('close', (code: number) => {
    ...
    private async stopUI(uiId: string) {
        try {
            const ui = this.uis.get(uiId);
            if (!ui) {
                throw new Error(`UI ${uiId} not found`);
